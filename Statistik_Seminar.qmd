...
- Bei der Berechnung der **Stichprobenvarianz** geht 1 Freiheitsgrad verloren:
  \[
  df = n - 1
  \]
  (weil der Mittelwert bereits aus den Daten berechnet wurde)
...
# X-squared: Chi-Quadrat-Wert
# df: Freiheitsgrade (6-1 = 5)
# p-value: Wahrscheinlichkeit unter H₀
```
...
1. **Daten sammeln**: Jeder würfelt 10 mal, Gruppe sammelt
2. **Eingeben**: `meine_daten <- c(...)`
3. **Testen**: `chisq.test(table(meine_daten))`
4. **Interpretieren**: Ist euer Würfel fair?
...
kontingenztab_beispiel <- table(
  Person = rep(c("A", "B"), each = 30),
  Würfelzahl = c(person_a, person_b)
)

print("Kontingenztabelle:")
print(kontingenztab_beispiel)
```

## Unabhängigkeitstest durchführen

```{r}
# Daten für Unabhängigkeitstest erstellen
set.seed(123)
person_a <- sample(1:6, 30, replace = TRUE)
person_b <- sample(1:6, 30, replace = TRUE, 
                   prob = c(0.1, 0.1, 0.2, 0.2, 0.2, 0.2))

# Kontingenztabelle für Unabhängigkeitstest
kontingenztab_unabh <- table(
  Person = rep(c("A", "B"), each = 30),
  Würfelzahl = c(person_a, person_b)
)

print("Kontingenztabelle:")
print(kontingenztab_unabh)

# Chi-Quadrat-Test für Unabhängigkeit
chi_unabhängigkeit <- chisq.test(kontingenztab_unabh)
print(chi_unabhängigkeit)

# Interpretation: 
# H₀: Würfelergebnis unabhängig von Person
# H₁: Würfelergebnis hängt von Person ab
```

## Residuen analysieren: Wo sind die Unterschiede?

```{r}
# Daten für Residuenanalyse erstellen
set.seed(123)
person_a <- sample(1:6, 30, replace = TRUE)
person_b <- sample(1:6, 30, replace = TRUE, 
                   prob = c(0.1, 0.1, 0.2, 0.2, 0.2, 0.2))

# Kontingenztabelle und Test für Residuen
kontingenztab_residuen <- table(
  Person = rep(c("A", "B"), each = 30),
  Würfelzahl = c(person_a, person_b)
)

chi_test_residuen <- chisq.test(kontingenztab_residuen)

# Standardisierte Residuen anzeigen
print("Kontingenztabelle:")
print(kontingenztab_residuen)

print("Standardisierte Residuen:")
print(chi_test_residuen$stdres)

# Große Residuen (|r| > 2) zeigen Zellen mit starken Abweichungen
# Positive Werte: Häufiger als erwartet
# Negative Werte: Seltener als erwartet
```
...
würfel_1 <- sample(1:6, 60, replace = TRUE)
würfel_2 <- sample(1:6, 60, replace = TRUE)
würfel_3 <- sample(1:6, 60, replace = TRUE, 
                   prob = c(0.05, 0.15, 0.15, 0.2, 0.2, 0.25))

alle_würfel <- data.frame(
  ergebnis = c(würfel_1, würfel_2, würfel_3),
  würfel_typ = rep(c("Typ1", "Typ2", "Typ3"), each = 60)
)

tab_homogenität <- table(alle_würfel$würfel_typ, alle_würfel$ergebnis)
chisq.test(tab_homogenität)
```
...
```{r echo=TRUE}
# Beispiel mit kleiner 2x2-Tabelle
kleine_tabelle <- matrix(c(2, 3, 1, 4), nrow = 2)
fisher.test(kleine_tabelle)
```
...
```{r}
# Beispiel-Kontingenztabelle für Phi
beispiel_2x2 <- matrix(c(10, 15, 8, 17), nrow = 2)
chi_beispiel <- chisq.test(beispiel_2x2)

# Phi-Koeffizient berechnen
phi <- sqrt(chi_beispiel$statistic / sum(beispiel_2x2))
phi
...
```{r echo=TRUE}
# Cramér's V installieren falls nötig: install.packages("vcd")
library(vcd)

# Daten für Cramér's V erstellen
set.seed(123)
person_a <- sample(1:6, 30, replace = TRUE)
person_b <- sample(1:6, 30, replace = TRUE, 
                   prob = c(0.1, 0.1, 0.2, 0.2, 0.2, 0.2))

# Kontingenztabelle für Cramér's V
kontingenztab_cramers <- table(
  Person = rep(c("A", "B"), each = 30),
  Würfelzahl = c(person_a, person_b)
)

print(kontingenztab_cramers)
...
### Balkendiagramme
```{r}
# Daten für Balkendiagramm erstellen
set.seed(123)
person_a <- sample(1:6, 30, replace = TRUE)
person_b <- sample(1:6, 30, replace = TRUE, 
                   prob = c(0.1, 0.1, 0.2, 0.2, 0.2, 0.2))

# Kontingenztabelle für Balkendiagramm
kontingenztab_balken <- table(
  Person = rep(c("A", "B"), each = 30),
  Würfelzahl = c(person_a, person_b)
)

# Gruppierte Balkendiagramme
barplot(kontingenztab_balken, beside = TRUE, 
        legend.text = TRUE, 
        col = c("lightblue", "salmon"),
...
### Mosaikplots
```{r echo=TRUE}
# Alle notwendigen Daten für Mosaikplot erstellen
library(vcd)
set.seed(123)
...
# Kontingenztabelle für Mosaikplot
kontingenztab_mosaic <- table(
  Person = rep(c("A", "B"), each = 30),
  Würfelzahl = c(person_a, person_b)
)

print(kontingenztab_mosaic)

# Mosaikplot erstellen
mosaic(kontingenztab_mosaic, shade = TRUE, 
       legend = TRUE, 
...
```{r echo=TRUE}
# Simulation verschiedener Würfeltypen
set.seed(123)

# Fairer Würfel
fair <- sample(1:6, 120, replace = TRUE)

# Leicht gezinkter Würfel (6 bevorzugt)
gezinkt <- sample(1:6, 120, replace = TRUE, 
                  prob = c(0.15, 0.15, 0.15, 0.15, 0.15, 0.25))

# Stark gezinkter Würfel
stark_gezinkt <- sample(1:6, 120, replace = TRUE,
...
```{r echo=TRUE}
# Beispiel: Meinung vor und nach Präsentation
# Matrix: (vorher_ja/nachher_ja, vorher_ja/nachher_nein,
#          vorher_nein/nachher_ja, vorher_nein/nachher_nein)
vorher_nachher <- matrix(c(25, 5, 10, 20), nrow = 2)
mcnemar.test(vorher_nachher)
```
