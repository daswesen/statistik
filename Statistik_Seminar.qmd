...
```{r echo=TRUE}
# Beispiel 2x2-Tabelle
behandlung <- matrix(c(40, 10, 20, 30), nrow = 2,
                    dimnames = list(
                      Behandlung = c("Neu", "Standard"),
                      Heilung = c("Geheilt", "Nicht_geheilt")
                    ))
print(behandlung)
```
...
```{r echo=TRUE}
# Odds Ratio manuell
a <- behandlung[1,1]  # Neue Behandlung, geheilt
b <- behandlung[1,2]  # Neue Behandlung, nicht geheilt  
c <- behandlung[2,1]  # Standard, geheilt
d <- behandlung[2,2]  # Standard, nicht geheilt
...
fisher.test(behandlung)$estimate
```
