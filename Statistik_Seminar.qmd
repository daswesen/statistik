...
> Du willst nach Berlin ziehen und findest 100 Wohnungsanzeigen. Dort stehen jeweils Wohnfläche (qm) und Miete (Euro).
>
...
> Beispielausgabe: Für jede zusätzliche m² steigt die Miete im Schnitt um 12,50 €.
...
> Erwartung: Punkte gleichmäßig um die Nulllinie verteilt
...
> Frage: Öffnet jemand eine E-Mail oder nicht? (Y = 1 oder 0)

- X: Betrefflänge in Zeichen
- Y: geöffnet (1) oder nicht (0)
- → **Ziel:** Wahrscheinlichkeit modellieren, nicht Mittelwert!
...
# Beispiel-Datensatz für Mail-Öffnungen
set.seed(42)
mails <- data.frame(
  betrefflaenge = sample(10:80, 100, replace = TRUE),
  geoeffnet = rbinom(100, 1, prob = plogis((sample(10:80, 100, replace = TRUE) - 40)/10))
)

modell_logit <- glm(geoeffnet ~ betrefflaenge, data = mails, family = binomial)

mails$pred <- predict(modell_logit, type = "response")

ggplot(mails, aes(x = betrefflaenge, y = geoeffnet)) +
...
- **Steigung positiv:** längere Betreffzeile → höhere Öffnungswahrscheinlichkeit
- **Steigung negativ:** längere Betreffzeile → niedrigere Öffnungswahrscheinlichkeit
- Mit `exp(coef(modell_logit))` erhält man **Odds Ratios**
...
|--------------------|---------------------------|-----------------------------------|
| Zielvariable Y     | metrisch (z. B. Preis)    | binär (z. B. geöffnet ja/nein)    |
...
1. Datensatz: `kunden` mit `anzahl_mails` und `gekauft` (0/1)
2. Modell mit `glm(gekauft ~ anzahl_mails, family = binomial)`
3. Visualisieren & interpretieren
4. Odds Ratio berechnen

---

## Fazit

- Beide Modelle zeigen Zusammenhänge – aber in unterschiedlichem Kontext
- Die Wahl hängt von der **Zielvariablen** ab
- In Hausarbeiten: Fragestellung → Modellwahl → Ergebnis → Interpretation
