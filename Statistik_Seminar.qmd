...
-   `|>` ist die native R-Pipe (seit R 4.1)
-   `%>%` kommt aus dem `magrittr`-Package – beide funktionieren hier

## Ausprobieren mit iris

-   Klassischer Datensatz: 150 Iris-Blüten, 3 Arten
-   Bereits in R enthalten – kein Package nötig
-   Laden: `data(iris)` oder einfach `iris`
...
iris |>
  group_by(Species) |>
  summarise(
    mittlere_laenge = mean(Sepal.Length),
    n = n()
  ) |>
  arrange(desc(mittlere_laenge))
```
...
Sie wollen nach Berlin ziehen und finden 100 Wohnungsanzeigen. Dort stehen jeweils Wohnfläche (qm) und Miete (Euro).
>
> **Frage:** Kann man aus der Wohnfläche den Mietpreis vorhersagen?
...
- Achtung: ANOVA zeigt nur, _dass_ ein Unterschied vorliegt, nicht _wo_ genau
