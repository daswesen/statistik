...
| Sitzung | Thema                                      |
|---------|--------------------------------------------|
| 8       | Regression & Prinzipien der Visualisierung |
...
- **Indirekter Beweis**: Wir prüfen, ob die Daten unter H₀ unwahrscheinlich wären, nicht ob H₀ falsch ist
...
## Spezifität (1-**α)** und Sensitivität (1-**β)**
...
# Sitzung 8: Regression & Prinzipien der Visualisierung
...
- Verstehen, wann man logistische statt linearer Regression braucht
- Grundprinzipien guter Visualisierung kennen und anwenden
...
## Und wenn die Frage nur "ja oder nein" ist?

- Bisher haben wir eine **Zahl** vorhergesagt (Miete in Euro)
- Oft ist die Zielvariable aber nur **ja/nein**:
  - Wird ein Buch ausgeliehen oder nicht?
  - Ist eine E-Mail Spam?
  - Bricht jemand das Studium ab?
  - Überlebt ein Passagier den Untergang der Titanic?
- Dafür gibt es die **logistische Regression**

------------------------------------------------------------------------

## Unser Beispiel: Die Titanic

- 1.309 Passagiere, `survived`: 1 = überlebt, 0 = nicht überlebt
- Außerdem: Alter, Geschlecht, Ticketklasse, Ticketpreis ...
- **Frage:** Wovon hing das Überleben ab?
...
## Erster Versuch: einfach wieder eine Gerade?
...
## Warum die Gerade hier nicht funktioniert

- Die Punkte liegen nur auf 0 oder 1 – die Gerade läuft mittendurch
- Beim teuersten Ticket (£512) sagt sie **1,48** vorher
- Eine Überlebenswahrscheinlichkeit von 148 %?
- Wir brauchen etwas, das **immer zwischen 0 und 1** bleibt

------------------------------------------------------------------------

## Die Lösung: eine S-Kurve
...
## Das ist die logistische Regression

- Die S-Kurve bleibt **immer zwischen 0 und 1**
- Das Ergebnis lesen wir als **Wahrscheinlichkeit**
- Gute Nachricht: In R ändert sich fast nichts

```{r}
#| echo: true
modell <- glm(survived ~ age + sex + factor(pclass),
              family = binomial,
              data = titanic)
```

- Aus `lm()` wird `glm()` plus `family = binomial` – das war's

------------------------------------------------------------------------

<!-- Screenshot stammt aus der Kaggle-Version des Datensatzes (großgeschriebene Variablennamen) -->

## Die Ausgabe von summary(modell)

![](images/titanic-logit.png)

------------------------------------------------------------------------

## Interpretation ohne viel Mathe

- Vorzeichen lesen wie bei der linearen Regression: **Minus = Wahrscheinlichkeit sinkt**
- `sexmale` negativ → Männer überlebten seltener
- `pclass 2/3` negativ → je schlechter die Klasse, desto geringer die Chance
- `age` negativ → Ältere überlebten seltener
- "Frauen und Kinder zuerst" – die Daten bestätigen es
- Wie groß die Effekte genau sind (Odds Ratios): Selbststudium für Neugierige

------------------------------------------------------------------------

## Konkrete Vorhersagen mit predict()
...
predict(modell, newdata = neue_passagiere, type = "response")
```
...
- 30-jährige Frau, 1. Klasse: ca. **92 %** Überlebenswahrscheinlichkeit
- 30-jähriger Mann, 3. Klasse: ca. **9 %**
- Aus den Koeffizienten werden konkrete Wahrscheinlichkeiten – das ist die Stärke des Modells
...
|--------------|---------------------|-------------------------------|
| Zielvariable | Zahl (z. B. Miete)  | ja/nein (z. B. überlebt)      |
| Ergebnis     | vorhergesagter Wert | Wahrscheinlichkeit (0–1)      |
| R-Funktion   | `lm()`              | `glm(..., family = binomial)` |
...
# Session 9: Kognitive Verzerrungen

## Bestätigungsfehler

**Was ist es?**

Man sucht, interpretiert und erinnert Informationen bevorzugt so, dass sie die eigene Ausgangshypothese bestätigen.

. . .

**Beispiel**

Eine Bibliothekarin glaubt, dass Nutzer:innen keine Schulungen brauchen – und notiert nur die Fälle, in denen Suchen auf Anhieb klappen.
...
Der erste genannte Wert (der »Anker«) beeinflusst alle nachfolgenden Schätzungen unverhältnismäßig stark.

. . .

**Beispiel**

Im Nutzerbefragungsbogen steht als Einstiegsfrage: »Nutzen Sie das Archiv mehr als 10× pro Monat?« – folgende Angaben orientieren sich an dieser Zahl.
...
Leicht erinnerbare Ereignisse werden als häufiger oder wahrscheinlicher eingeschätzt als statistisch gerechtfertigt.

. . .

**Beispiel**

Weil zuletzt drei Titelseiten über Datenpannen erschienen, überschätzen Mitarbeitende das tatsächliche Risiko eines Angriffs auf das eigene System.
...
Nach Bekanntwerden eines Ergebnisses glaubt man, es von Anfang an gewusst zu haben (»Ich hab's doch gesagt!«).

. . .

**Beispiel**

Nach dem Rückgang der Printausleihe um 40 % sind alle überzeugt, den Trend schon vor Jahren vorhergesagt zu haben.
...
Nur die »Überlebenden« eines Prozesses werden analysiert; die ausgesonderten Fälle bleiben unsichtbar.

. . .

**Beispiel**

Studie zu »erfolgreichen Open-Access-Zeitschriften« – die Dutzende Journals, die nach zwei Jahren eingestellt wurden, tauchen in der Analyse nicht auf.
...
Die Stichprobe repräsentiert die Grundgesamtheit nicht systematisch – bestimmte Gruppen sind über- oder unterrepräsentiert.

. . .

**Beispiel**

Eine Online-Umfrage zur Bibliotheksnutzung erreicht nur internetaffine Personen unter 40; ältere Nutzer:innen ohne Smartphone fehlen komplett.
...
Die Personen, die nicht antworten, unterscheiden sich systematisch von denen, die antworten – das Ergebnis ist verzerrt.

. . .

**Beispiel**

Zufriedene Bibliotheksbesucher:innen füllen den Feedback-Bogen seltener aus als unzufriedene; die Auswertung zeigt ein zu negatives Bild.
...
Befragte erinnern sich selektiv oder ungenau, was zu systematischen Fehlern in retrospektiven Erhebungen führt.

. . .

**Beispiel**

Befragte unterschätzen, wie oft sie eine Datenbank im vergangenen Monat genutzt haben – tatsächliche Logins laut Serverlog liegen doppelt so hoch.
...
So lange verschiedene Tests, Variablen oder Subgruppen ausprobieren, bis ein signifikantes Ergebnis (p \< 0,05) erscheint – dann nur dieses berichten.

. . .

**Beispiel**

Von 20 getesteten Schlagwortkombinationen zeigt eine zufällig p = 0,04 – diese wird veröffentlicht, die anderen 19 Versuche nicht erwähnt.
...
**H**ypothesizing **A**fter **R**esults are **K**nown: Eine Hypothese wird erst nach dem Sehen der Daten formuliert, aber als ursprünglich geplant ausgegeben.

. . .

**Beispiel**

Nach Auswertung der Nutzungsdaten fällt ein Muster auf – dieses wird nachträglich als »Forschungsfrage« in den Bericht hineingeschrieben.
...
Ein Modell passt sich so stark an die Trainingsdaten an, dass es auf neuen Daten versagt – es lernt Rauschen statt Muster.

. . .

**Beispiel**

Ein Empfehlungsalgorithmus für Bibliotheksmedien erzielt 95 % Treffsicherheit auf historischen Daten, empfiehlt Neuzugängen aber nur Klassiker aus dem Vorjahr.
...
Eine wichtige Einflussvariable fehlt im Modell – der geschätzte Effekt anderer Variablen wird dadurch verzerrt.

. . .

**Beispiel**

Studie zeigt: Mehr Bibliotheksbesuche → bessere Noten. Aber »Lernmotivation« fehlt als Variable; sie erklärt beides.
...
Nur die Datenpunkte oder Zeiträume werden gezeigt, die die gewünschte These stützen – ungünstige Daten werden verschwiegen.

. . .

**Beispiel**

Der Jahresbericht zeigt den Ausleihzeitraum Oktober–Dezember – in dem traditionell die höchsten Zahlen anfallen. Der schwache Sommer bleibt unerwähnt.
...
Die Y-Achse beginnt nicht bei null, sodass kleine Unterschiede dramatisch groß wirken.

. . .

**Beispiel**

Ein Balkendiagramm beginnt bei 9.800 statt 0 – ein Unterschied von 50 Ausleihen sieht aus wie eine Verdoppelung.
...
Ein Trend, der in allen Teilgruppen gilt, verschwindet oder kehrt sich um, wenn die Gruppen zusammengefasst werden.

. . .

**Beispiel**

Datenbankschulungen wirken bei Erst- und Drittsemestern separat positiv auf die Recherchegüte – aggregiert sieht man keinen Effekt, weil Drittsemester seltener teilnehmen.
...
Aus Gruppenmerkmalen werden unzulässig Schlüsse auf Individuen gezogen.

. . .

**Beispiel**

Stadtteile mit hoher Bibliotheksdichte haben niedrigere Arbeitslosigkeit → Schluss: Bibliotheksnutzer:innen sind seltener arbeitslos. Falsch.
...
Positive oder signifikante Ergebnisse werden häufiger veröffentlicht als Null-Ergebnisse – das publizierte Bild ist systematisch schiefer als die Realität.

. . .

**Beispiel**

Meta-Analyse zur Wirksamkeit von Informationskompetenz-Schulungen findet kaum Studien ohne Effekt – obwohl viele solcher Studien durchgeführt, aber nie eingereicht wurden.

------------------------------------------------------------------------

## Messfehler-Verzerrung

**Was ist es?**

Systematische Fehler im Messinstrument führen dazu, dass die Realität konstant über- oder unterschätzt wird.

. . .

**Beispiel**

Der Zähler am Bibliothekseingang erfasst nur Eintretende, nicht Austretende – Doppelzählungen bei Drehkreuz-Pannen verzerren alle Besuchsstatistiken.
...
Identische Information führt zu unterschiedlichen Entscheidungen, je nachdem wie sie präsentiert wird.

. . .

**Beispiel**

»90 % der Anfragen werden beantwortet« klingt besser als »1 von 10 Anfragen bleibt unbeantwortet« – obwohl es dieselbe Aussage ist.
...
::: incremental
- Wie wurde die Stichprobe erhoben? **Wer fehlt?**
- Welche Hypothese hatte ich vorher – und wie beeinflusst sie meine Auswertung?
- Zeige ich **alle** Daten oder nur die bequemen?
- Könnte eine fehlende Variable mein Ergebnis erklären?
- Wie würde jemand, der anderer Meinung ist, dieselben Daten lesen?
:::
